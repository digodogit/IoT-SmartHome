...
export async function editDisp(newDispData: unknown) {
  ...
  if (!validatedDisp.success) {
    return {
      message: "dados para criar dispositivo invalido",
    };
  }
  console.log(validatedDisp.data?.components);
  const response = await fetchServerData(
    "/dispositivos/editDisp",
    "POST",
    {
  ...
  if (!response) {
    return {
      message: "dispositivo data invalido",
    };
  }
...
export async function createDisp(disp: unknown) {
  ...
  if (!response) {
    return {
      message: "não foi possivel criar dispositivo",
    };
  }
  revalidatePath("/dashboard", "layout");
